...
/*   Updated: 2024/06/26 13:57:31 by xroca-pe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int handle_operators_syntax(t_token *token, t_syntax_state *state, t_shell *shell)
{
    if (token->type == PIPE || token->type == AND || token->type == OR)
    {
        if (state->last_was_operator || state->last_was_pipe || state->last_was_redirect || state->last_was_logical || !state->last_was_word)
        {
...
static int handle_redirects_syntax(t_token *token, t_syntax_state *state, t_shell *shell)
{
...
static int handle_parentheses_syntax(t_token *token, t_syntax_state *state, t_shell *shell)
{
...
static int handle_word_syntax(t_token *token, t_syntax_state *state, t_shell *shell)
{
    if ((state->last_was_operator || state->last_was_pipe || state->last_was_redirect) && token->value[0] == '&')
    {
        handle_error("syntax error: unexpected '&'", shell);
        return (0);
    }
...
static int handle_general_tokens_syn(t_token *token, t_syntax_state *state, t_shell *shell)
{
    if (token->type == PIPE || token->type == AND || token->type == OR)
    {
